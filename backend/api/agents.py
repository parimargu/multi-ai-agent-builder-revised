...
async def save_workflow(
    ...
    existing_nodes = await db.execute(select(AgentNode).where(AgentNode.agent_id == agent_id))
    for node in existing_nodes.scalars().all():
        await db.delete(node)

    existing_edges = await db.execute(select(AgentEdge).where(AgentEdge.agent_id == agent_id))
    ...
    # Create new nodes
    id_map = {}  # temp_id (from frontend) -> real UUID
    for node_data in data.nodes:
        node = AgentNode(agent_id=agent_id, **node_data.model_dump(exclude={"parent_node_id"}))
        db.add(node)
        await db.flush()
        # Store mapping using label as temp key if needed
        id_map[str(node.id)] = node

    # Create new edges
    for edge_data in data.edges:
        edge = AgentEdge(agent_id=agent_id, **edge_data.model_dump())
        db.add(edge)
