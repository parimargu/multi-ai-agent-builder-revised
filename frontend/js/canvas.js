...
const Canvas = {
    ...
    init() {
        ...
        this.canvasArea.addEventListener('mousedown', (e) => {
            if (e.target === this.canvasArea || e.target === this.svgEl) {
                this.isPanning = true;
            ...
            <div class="palette-category" data-category="${cat.key}">
                ...
                ${cat.items.map(item => `
                    <div class="palette-node" draggable="true"
                         ...
                         onmousedown="Canvas.onPaletteNodeClick('${item.subType}')">
                        <div class="palette-node-icon" style="background:${item.color}22;">
                            ${item.icon}
                        </div>
...
function filterPaletteNodes(query) {
...
async function saveWorkflow() {
    ...
    try {
        // First save nodes individually and collect real IDs
        const existingAgent = await API.getAgent(Canvas.agentId);
        
        // Delete existing nodes and edges
        for (const edge of (existingAgent.edges || [])) {
            await API.deleteEdge(Canvas.agentId, edge.id);
        }
        for (const node of (existingAgent.nodes || [])) {
            await API.deleteNode(Canvas.agentId, node.id);
        }

        // Create new nodes
        const idMap = {}; // local id -> server id
        for (const node of Canvas.nodes) {
            const serverNode = await API.addNode(Canvas.agentId, {
                node_type: node.type,
                sub_type: node.subType,
                label: node.label,
                position_x: node.x,
                position_y: node.y,
                config: node.config || {},
            });
            idMap[node.id] = serverNode.id;
            node.id = serverNode.id;
        }

        // Create edges with mapped IDs
        for (const edge of Canvas.edges) {
            const srcId = idMap[edge.sourceId] || edge.sourceId;
            const tgtId = idMap[edge.targetId] || edge.targetId;
            await API.addEdge(Canvas.agentId, {
...
async function executeWorkflow() {
    ...
    try {
        const result = await API.executeAgent(Canvas.agentId, { message: 'Hello, execute this workflow.' });
